...
def get_page_url(url):
    ...
    page_url = []
    for item in soup.find_all('div', class_="play-btns clearfix"):
        for player in item.find_all('div', class_="player"):
...
def get_audio_url(url ):
    ...
    html = askURL(url)
    regex_urlpath = r'var urlpath = "(.+?)\"'
    url_path = re.findall(regex_urlpath, html)
    web_url = url.split('/')[0]
    flv_url = web_url + '/playdata/' + url_path[0] + '.html'
    html_flv = askURL(flv_url)

    load_dict = json.loads(html_flv)
    flv_url_path = load_dict['urlpath'] 
    
    # 获取文件地址
    mp3_url = flv_url_path.replace('flv', 'mp3')
    # 获取文件名 
    soup = BeautifulSoup(html, "html.parser")
    title = soup.find(attrs={"name":"keywords"})['content'] 
    title = title.replace(',', '')
    detail = [mp3_url, title, url_path[0]]
...
def get_download_audio(url, file_path, con):
    # 获取指定url的音频列表
    audio_list = get_page_url(url)
    # html = askURL(url)

    # 获取指定url的音频详情
    for audio in audio_list:
        time.sleep(2)
        audio_detail = get_audio_url(audio)
        if audio_detail[0] != '' and audio_detail[1] != '' and audio_detail[2] != '':
            ...
            if is_exist(id, con):
                ...
                sql = "insert into pingshu(id,title,url,file_path,can_down) values('" + id + "','" + title + "','" + detail_url + "','" + absolute_path + "'" + "," + str(
                    can_down_str) + ")"
...
def is_exist(id, con):
    sql = "select * from pingshu where id = '" + id + "'"
...
def main(url, file_path):
    ...
    #     get_download_audio(page_url, file_path, con)
...
if __name__ == '__main__':
    # 爬取的网址
    url = 'http://wangyuebo.zgpingshu.com/3245/'
    # 保存的目录
    file_path = os.getcwd()  # 当前工作目录。
    file_path = file_path + '\\wangyuebo\\duanpian'
    # 创建文件夹
